...
const seedTeacher = async (req, res, next) => {
    ...
    if (schoolObj !== undefined) {
        ...
        if (
            teacherGrades[0] !== "all" &&
            teacherGrades[0] !== "self-contained" &&
            teacherSubjects[0] !== "self-contained" &&
            teacherSubjects[0] !== "all"
        ) {
...
const seedSubjects = async (req, res, next) => {
    ...
    res.sendData({
        msg: "Seeded school subject data successfully " + schoolSubjects
    });
};
const getSchool = async schoolStr => {
    return school.findOne({
        include: [
            { model: grade, as: "grades", required: false },
